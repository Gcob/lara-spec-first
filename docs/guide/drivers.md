...
with no fork and no second class, because the difference between them is three lines of config, not a different driver.
Structure is what is worth writing code for, because structure is what several projects share.

### When a feature needs a driver

Customization elsewhere in this documentation is driven by nomenclature: an `operationId`
[derives a class name](./code-generation/generated-file-anatomy.md#deriving-a-name-without-operationid), a
`securitySchemes` name [matches a guard](./security.md#scheme-names-match-guard-names), a factory override is
[found by what it `extends`](./code-generation/response-dtos.md#overriding-a-factory-by-extending-it). Those work
because a name means the same thing everywhere it appears.
...
**Writing a driver is an ordinary Laravel extension, registered from the consumer's own service provider.** Not a config
key naming a class to be reflected into, and not a directory this package scans: `extend()` from a provider is the idiom
Laravel already uses for a custom cache store, queue connection or filesystem disk, so it costs no new concept and every
Laravel developer already has the reflex.
...
**Each driver-based feature publishes both.** The interface is the contract and the thing to type against. The abstract
class implements what every driver of that feature would otherwise rewrite: reading the mapping, resolving a configured
key, normalizing what comes back. A custom driver only writes the part that is actually custom: where to look in the
document.

It is the [same split as the generated two layers](./code-generation/index.md#an-interface-and-an-abstract-class), for
the same reason: the interface is what makes a substitution safe, and the abstract class is what makes writing one
cheap. A driver author may ignore the abstract class and implement the interface directly; nothing depends on the base
class being used.
...
reports which driver is in effect for each feature, third-party ones included, in its configuration section. Printing
...
- **A driver resolves at build time, and what it resolved is baked into what the build emits.** Registering a driver in
  ...
  Artisan command, so a provider-registered driver is available to it, and the ordering works out without an exception
  to that rule. Where a feature does need the driver at request time, that is stated in the feature's own document and
  is a different claim from this one.
- **A driver never invents a diagnostic of its own.** A misconfigured mapping, an unknown driver name, a driver that
  ...
  with the [document position](./doctor.md#what-the-doctor-guarantees) like any other finding. A third-party driver that
  writes to the log instead has quietly opted its users out of rule 2.

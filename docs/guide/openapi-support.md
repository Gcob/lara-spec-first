...
| An absent keyword is indistinguishable from one written with its default value.                                                      | `SpecBaseObject.php:361-393`, `Schema.php` `attributeDefaults()`                                                                                                                                                                                                                                                              | `__get()` falls back to `attributeDefaults()` and `hasPropertyValue()` is `protected`, so from our side an absent `additionalProperties` reads `true` exactly as an explicit `additionalProperties: true` does. The same holds for `nullable`, `exclusiveMinimum` and `exclusiveMaximum`, and for any boolean through the typed fallback. `__isset()` does not help: it returns `__get() !== null`, true for a defaulted key. The way out is `getSerializableData()`, which returns the written properties with no defaults folded in.                                                                                                                                                                                                                                                                                                                                                                                    |
...
- **`exclusiveMinimum` and `exclusiveMaximum` carry two meanings under one name**, and only the version strategy ever
  sees the raw form.
- **An absent keyword and one written with its default value are indistinguishable**, which is what makes the
  `additionalProperties` rows below a question about a value the parser cannot tell you was written.
...
detects later, since nothing fails. #34 owns that refusal and tests it.
...
| Construct                                     | Level        | Note                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| --------------------------------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `type`                                        | Deferred     | Normalized to a list behind the [version strategy](#handling-30-and-31), so nothing downstream branches on the document version. #34.                                                                                                                                                                                                                                                                                                          |
| `nullable` (3.0), `type: [..., "null"]` (3.1) | Deferred     | Two spellings of one idea. Which one the normal form takes is #34's to state; that it is one form is settled here.                                                                                                                                                                                                                                                                                                                             |
| `enum`                                        | Deferred     | Phase 2, as an `in` rule.                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `const`                                       | Deferred     | **Raw**, and data-carrying, so the refusal above does not reach it. 3.1's one-value `enum`, and treated as one.                                                                                                                                                                                                                                                                                                                                |
| `format`                                      | Deferred     | Phase 2, and only where Laravel's rule means the same thing: `email`, `uuid`, `ipv4`, `ipv6` and `date` map directly; `date-time` needs `date_format` with an RFC 3339 pattern, since Laravel's `date` accepts what `strtotime` accepts; `uri` is **not** Laravel's `url`, which turns away the non-hierarchical URIs (`urn:…`) JSON Schema allows. Every other value is an annotation, reported once per document rather than per occurrence. |
...
| `example`, `examples`                         | Deferred     | Read for the mocker, never for validation. `examples`, the 3.1 list, is **Raw**. Both spellings can appear, and precedence between them is part of the normal form #34 defines.                                                                                                                                                                                                                                                                |
...
| `exclusiveMinimum`, `exclusiveMaximum` | Deferred | Normalized to one spelling behind the strategy, which #34 picks. A number carries what a boolean needs a sibling to mean, and the raw form never leaves the strategy.                                                                                                                                                               |
...
| Construct                             | Level   | Note                                                                                                                               |
| ------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `contentMediaType`, `contentEncoding` | Ignored | **Raw**, and both carry a string, so no pointer can hide in them.                                                                  |

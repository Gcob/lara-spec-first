...
| A `$ref` whose JSON pointer lands inside data our own cycle guard correctly treats as opaque exhausts memory the same way a pure cycle does. | Verified: `tests/Fixtures/ref-inside-example.yaml` and `tests/Fixtures/example-object-value.yaml`, each accepted by `ReferenceCycleDetector` and each dying under `OperationExtractor::parse()` with _Allowed memory size exhausted_, `Reference.php:246` / `Reference.php:328` recursing without ever reaching a schema | A second way into the same failure as the row above, and one our guard cannot see by its own design: `#/components/schemas/A/example` is a legal pointer, `example` is legitimately data and is not walked, and there is no `$ref`-shaped node anywhere in the chain our guard follows. Only the parser, resolving the pointer against the already-built object tree, finds the `$ref` sitting inside that data and loops on it. **Not yet guarded against — pinned rather than silently worked around.** Unrecoverable, so it is not a `toThrow()` case: `tests/Conformance/KnownParserBugsTest.php` runs it in a child process and asserts the exit code and stderr instead, the same way `tests/Support/extract.php` and the "Subprocess assertions" row in [`stack.md`](../project/stack.md) describe. |
...
take the process down, and `components.pathItems` loses an endpoint without a word. All three were found within days of
first use, on a surface no wider than paths and references. That is worth writing down honestly rather than discovering
again later — and the cycle shape found through the conformance suite itself, rather than through production use, is the
clearest argument yet for why that suite exists.
...
- **The list of data-carrying keys reasons about names, never about positions.** A schema property genuinely named
  `default`, `example`, `enum` or `const` is a Schema Object and is not followed. A false negative, and the harmless
  direction, but real.
...
[API design is a skill](./code-generation/index.md#naming-and-the-rename-problem) point the same way — a build that
refuses `{user-id}` and says _rename this parameter to `user_id` in your specification_ is teaching a real constraint of
the platform, once, at build time. A build that silently converts is maintaining a shadow naming scheme forever, and the
...
- `operationId`: optional in the specification, not guaranteed unique, not guaranteed to be a valid PHP identifier — and
  ...
  answered in [`code-generation/index.md`](./code-generation/index.md#naming-and-the-rename-problem); what remains here
  is how a missing or unusable `operationId` is reported.
...
| `tags`                                  | Partial      | No routing effect, but they are the author's own grouping of their contract, and the package reuses it rather than inventing one — see [scaffolding output](./code-generation/index.md#the-build-names-the-command-instead-of-running-it).                                                                                                                                                                                                                                                                                                                                    |
...
| `operationId`                           | Partial   | Names the generated controller and method. **Required on `public` + `stable` operations**; elsewhere the [method and path](./code-generation/index.md#when-operationid-is-absent-derive-from-method-and-path) stand in for it. |

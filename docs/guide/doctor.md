...
| Lifecycle         | The [`x-sunset` and `x-lifecycle` rules](./lifecycle.md#the-doctor-rules-that-follow), plus the coverage report: how many public operations are actually `stable`, and therefore how much of the API is protected at all. A removal date that has passed is a finding; one that is merely approaching is reported beside the coverage report and never touches the exit code, so a run cannot fail on a day nobody changed anything.                                                                                                                                                                                                                                                                   |

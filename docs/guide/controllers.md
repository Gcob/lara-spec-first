...
**Every operation gets its own generated controller, carrying exactly one route method, always named `routeAction`.**
Not a resource controller holding several operations, and not an invokable class.
...
**`routeAction` declares one parameter per path parameter, `string`, named the way the specification names it.**
`GET /users/{id}` generates `routeAction(string $id): mixed`.

**PHP is what makes this a decision rather than a detail.** An override may not add a required parameter, so a parent
declaring none would forbid every custom controller from ever seeing `{id}`, and a developer could only reach it through
...
[still normalized](./code-generation/generated-file-anatomy.md#identity-is-the-path-and-method), because that question
...
a bound model, and the parent will declare that type when it does. A child may narrow the return type, `array` where the
parent says `mixed`, but not the parameters, which is ordinary PHP variance rather than a rule of this package.
...
**`x-controller` on an operation carries the fully-qualified name of its custom controller**, the same way PHP itself
writes a class reference. It is not a grouping key and it does not name a base class to extend. It names _this
...
[actively encourages adding one](./code-generation/generated-file-anatomy.md#deriving-a-name-without-operationid), which
...
child, created once. **The build does not scan for the child, because the specification already told it the name**, so
resolution is a lookup rather than a search. That is simpler than
[the factory override mechanism](./code-generation/response-dtos.md#overriding-a-factory-by-extending-it), which has to
scan configured directories precisely because nothing in the specification names a factory override.
...
again. That is the structure; [the request path](#reads-need-no-code-at-all) is the same layers seen from a request
arriving.
...
and [settling those names is still open](#the-crud-semantic-is-an-empty-marker). It is drawn from what this document set
claims today, so read it for how the pieces fit and read the section that owns a name before depending on it. Expect to
revisit it rather than to inherit it.
...
still share a short name, `…\Admin\UserController` and `…\Api\UserController`, and silently letting one generated parent
serve two operations is the kind of guess this package refuses everywhere else. Its message is its own rather than the
collision message an `operationId` gets, because the fix is a different one: neither of these operations is being named
from an `operationId`, so advice about that key would send a reader looking for something they do not have.
...
**That one line is a call to the parent, and it is not decoration.** A method with an empty body returns `null`, which
Laravel renders as an **empty `200`**, so an empty scaffold would quietly turn the operation's honest `501` into a lie,
in the one command whose whole job is to help. The parent call keeps the `501` until the developer replaces it, and
replacing it is exactly what implementing the operation means:
...
[points the route at the child](#two-classes-found-by-name), since that target is resolved at build time. **This is not
[the invariant](./code-generation/index.md#a-build-never-destroys-your-work) in reverse:** the rule is that the build
never creates a class you will own, and nothing says the command that does may not ask the build to catch up. It is
skipped after a declined bulk confirmation, because a refusal is respected whole.

#### The scaffold is not a publishable stub

**The scaffold is not a publishable stub, and what a stub would have to leave alone is the reason.** Every Laravel
generator worth copying lets a project publish its stubs, so the absence is a choice rather than an omission.
...
[PHP forbids a child from widening it](#the-signature-is-the-contract). A stub can hold placeholders for those, but a
stub whose placeholders are all mandatory is a template with one editable region: the comment.
...
a published stub cannot silently unhook a class from its own operation. Until then the answer to wanting a different
file is that the file is yours: `spec:make` writes it once and never touches it again.

#### The command offers to write it

**`spec:make` prints the extension to add, names the exact line, and offers to insert it, defaulting to no.**
...
decision is required. Answering yes runs the insertion below, and then scaffolds the class and builds: one command from
an operation the contract says nothing about to a class the route reaches.

**A developer who does not want to be asked types `--yes`, never `--force`.** `--tag=` and `--all` are the same
primitive run several times, one file per operation, since nothing this package creates is a grouped file, and both list
what they would create and ask before creating anything, defaulting a non-interactive run to no. `--yes` answers that
confirmation and the extension prompt above with the answer the command already proposed, including under
`--no-interaction`, which is the whole reason the flag exists: an explicit `--yes` on the command line _is_ somebody
...
there is nothing to parse for: the operation's line is known, and one line goes in beneath it at the matching depth,
with nothing parsed, reformatted or reflowed.
...
Inheritance carries exactly one thing here, the two-class parent-child seam above, and nothing else. `SpecController` is
one abstract base, shared by every operation, and it stays deliberately thin: no per-operation knowledge, because it
cannot have any.
...
**`routeAction` is one line, calling a named CRUD method the build chose.**
...
identical, so the indirection buys nothing there and is kept only for consistency. On a write it is what the split is
for:
...
**`x-model` is the switch for the whole persistence layer.** Declaring it gives an operation a model contract, a default
query, CRUD defaults and a DTO factory. Declaring nothing gives it none of them, and its `routeAction` throws
...
### An interface, and a trait beside it

**An interface declares the model contract, and a trait ships the implementation that can be shared.** The interface is
the type, and a trait is not one, which is the whole reason both exist:
...
**This interface carries real methods, unlike [the CRUD markers below](#the-crud-semantic-is-an-empty-marker), and the
difference is not a matter of taste.** `getModelClass()` and `getQuery()` take **no parameters**, so an implementation
can satisfy them exactly and a child can override them freely. `respondWithSingle(User $user)` does take one, and PHP
requires parameter types to be contravariant, so an interface declaring `respondWithSingle(Model $model)` would forbid
the narrowly-typed generated method above. **A parameterless method can be a contract; a method that receives the bound
...
the trait so its default exists in one place rather than repeated across every model-backed controller, and it stays the
seam a project overrides most often: a global scope, eager loading, hiding soft-deleted rows are all legitimate query
construction that never contradicts what the specification declared. Both, and the trait, are drawn in
[the seam diagram](#two-classes-found-by-name) above.

### The CRUD semantic is an empty marker

**Alongside `HasModel`, the generated class implements an empty interface naming the CRUD semantic the build detected**,
one interface per semantic, each declaring no methods at all.

Empty follows directly from [the parameterless rule above](#an-interface-and-a-trait-beside-it): a CRUD method receives
...
means to leave open. **The contract that binds is `routeAction`'s return type**, and PHP already enforces that.
...
[the seam diagram](#two-classes-found-by-name), where the generated class implements exactly one of them.
...
**Open (#41):** the names. `HasModel`, `InteractsWithModel`, `CreatesResource`, `UpdatesResource`, `DeletesResource`,
...
departs from these semantics is free to declare `x-controller` and write whatever the operation actually does, which is
...
**`SpecController` implements Laravel's own `HasMiddleware` interface, with a default `middleware()` returning
...
One level below [the two-class seam](#two-classes-found-by-name): the same classes, in the order a request reaches them.
Every hop after the route is a method a child may override, and the phase note names the three participants that do not
exist yet.
...
**This package ships no filtering or sorting.** OpenAPI has no vocabulary for a filter DSL any more than it does for
[row-level authorization](./security.md#row-level-rules-are-a-policys-job), and the same test applies: inventing one
would be a far larger commitment than anything else in this document set, for a feature every serious Laravel project
already reaches for a dedicated package to solve. `getQuery()` is already the hook: override it and wire in Spatie's
Query Builder, Scout, or whatever the project already uses.

## Writes, by the same default

**`create`, `update` and `delete` get the same treatment as reads**: a generated default built from `x-model` and the
...
**No opt-out, and no way to force `501` on a write `x-model` already gates.** The tempting worry is a `POST /orders`
that saves the row and returns `200` while nothing charged the card. That is not a defect in this default. It is the
boundary this package has been honest about from the start. **Neither the specification nor this package is magic.**
They own how a client talks to the application over HTTP; a developer owns what happens once a request arrives, exactly
as [`security.md`](./security.md#row-level-rules-are-a-policys-job) draws the same line for authorization. A flag to
suppress the default would only protect a developer who shipped `x-model: Order` on a payment-charging endpoint without
ever opening the generated file, and building for that case would mean designing around the wrong audience rather than
...
[the interface family that can carry a real contract](#an-interface-and-a-trait-beside-it) rather than to the empty
markers, the same rule applied again.
...
## The doctor counts two things

**The coverage report separates `501` from everything that works, and stops there.**
...
- **A custom child whose `x-controller` value has changed**, leaving it extending a parent the build no longer emits.
  ...
  while the doctor is already reading the tree to judge it and pays nothing extra for the question.
...
- The [interface, trait and method names](#the-crud-semantic-is-an-empty-marker), all of which are public API surface
  under [rule 4](./openapi-support.md#the-four-rules) from the first release on.

...
> - OpenAPI has no vocabulary for pagination, so a paginated endpoint is an ordinary one until you name a driver.
> - One driver ships built in, `laravel`, matching Laravel's own paginator envelope.
> - The build writes one envelope DTO per paginated operation, beside its item DTO and its metadata DTO.
> - An operation can paginate with no Eloquent model at all, which is the case the two seams exist to prove.
...
**One interface, `PaginationAdapterInterface`**, and everything downstream of it asks that interface rather than knowing
...
**pagination runs in both directions.** A rate limit is read and reported. A page is _requested_, so the adapter has to
say which query parameters carry the page and the size for the generated controller to accept them, **and returned**, so
the adapter also has to say where the collection and the totals sit in the response envelope. Working names, one per
question:
...
use, the shape that eventually grows a `match` statement per caller. Rather than invent anything, the adapter hands back
...
**There is no default convention, so pagination support does nothing until the driver and the mapping are configured.**
Not a driver that guesses, and not a driver picked for you: an unconfigured `pagination` block means the package treats
paginated endpoints as ordinary ones, which is exactly what it does today.

This is the same posture as [the remote reference allowlist](./remote-references.md), empty until a project declares a
host, and [the factory override scan](./code-generation/response-dtos.md#overriding-a-factory-by-extending-it), which
scans nothing until a project names a directory. A feature whose whole premise is _we cannot know your convention_ has
no business assuming one on your behalf.
...
driver does not cover [registers its own](./drivers.md#turnkey-by-default-yours-to-replace) from its service provider.
What follows here is only what is specific to pages.

**The driver is configured, not detected**, and one ships with the package, the one this package can actually justify,
which is Laravel's own paginator output:
...
**The build emits one envelope DTO per paginated operation, alongside the item DTO and a metadata DTO.**
...
**Two seams, and only one of them depends on `x-model`:**
...
**`getPaginator()` returns one of Laravel's own pagination contracts**, not a type this package invents. It costs no new
concept, which is the same argument that put middleware on
[Laravel's `HasMiddleware`](./controllers.md#middleware-is-a-method) rather than on an interface of ours. And a page can
be built from anything, so an operation with no Eloquent behind it is not shut out:
...
So `getPaginator(): Paginator` would mechanically exclude cursor pagination, the third of the three styles this document
set out to cover. **The return type is the union `Paginator|CursorPaginator`**, which is what PHP gives us for two
unrelated interfaces and which stays honest about the fact that they are unrelated. An umbrella interface of our own
would be the alternative, and it loses: it would mean every third-party paginator has to implement ours before this
package will accept it, for no gain over naming both types.
...
mapping says what the parameters are called, the specification says they exist, and the doctor checks the two agree, the
same shape as [the security scheme naming contract](./security.md#scheme-names-match-guard-names).

## What this document does not cover

Three things a reader looking at a paginated endpoint will want, and will not find here:
...
- **Filtering, sorting and search.** OpenAPI has no vocabulary for a filter DSL, and inventing one is a far larger
  ...
  [`controllers.md`](./controllers.md#reads-need-no-code-at-all) says so in the same words.
...
- Whether [the doctor](./doctor.md) reports an operation that looks paginated while no driver is configured. It has no
  ...
  [the declared-parameters finding](#parameters-must-be-declared), which fires only once a driver has already
  established that the response is a page.

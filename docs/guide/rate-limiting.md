...
**One interface, `RateLimitAdapterInterface`, returning a list of windows**, each one a `final readonly RateLimitWindow`
...
`getLimit()`, `getRemaining()` and `getResetTime()`, read better for the single-window case, and they were the first
sketch here. But [multiple windows are supported from the start](#windows-are-first-class), and
...
neither built-in driver covers [registers its own](./drivers.md#turnkey-by-default-yours-to-replace) from its service
provider. What follows here is only what is specific to rate limits.

**The driver is configured, not detected**, exactly because detection has nothing reliable to key off. Two ship with the
package:
...
[driver the project writes itself](./drivers.md#turnkey-by-default-yours-to-replace), which is the mechanism's whole
...
[explained below](#windows-are-first-class). A `mapping` always describes windows, and one unnamed window is the common
case spelled without ceremony.

## Declare the 429 once, then `$ref` it
...
429 is the same response everywhere, while a paginated `200` wraps a different item type per operation and has much less
to share.

### Why the `$ref` is not imposed
...
by the time a driver reads a response it holds inlined content with no record of where it came from, and a referenced
...
because after resolution there is nothing left to compare.
...
a second `429` entry is not something a document can express. One response, one reference, one config block, so the
...
of one `RateLimit-Limit` plus a `RateLimit-Policy` describing the windows, there is nothing to duplicate at all.

The case that needs more is a project naming its windows separately, `X-RateLimit-Limit-Minute` beside
...
**A `mapping` always describes windows, plural, and several are supported from the first release**: per minute, per
hour, per day, per week, whatever a project publishes. Not deferred, and not a later addition, for the reason
[the interface section](#an-adapter-behind-one-interface) gives: third-party drivers implement against this shape, so a
dimension added afterwards costs a major version. It is cheaper to support many windows now than to support one and
regret it.
...
principle as [`x-audience` and `x-lifecycle` defaults](./lifecycle.md#the-audience-sets-the-default), which are resolved
...
- **Build-time:** the `extension` driver's mapped fields carry an actual numeric threshold in the specification, and the
  build reads it to attach Laravel's own `throttle` middleware to the generated route with the right limit, so the spec
...
[window dimension had to exist first](#windows-are-first-class): it is load-bearing for whichever of these two ships,
not a refinement on top of them.
...
- The config key names, `rate_limiting`, `driver` and `mapping`, are working names, not decided. Public API surface
  under [rule 4](./openapi-support.md#the-four-rules).
- Whether [the doctor](./doctor.md) validates the mapping (a required key missing, a driver name it does not recognize)
  ...
  [windows](#windows-are-first-class) the check has a second half worth stating: a configured window whose fields appear
  in no 429, and the reverse, a 429 declaring fields no configured window claims, which is a limit the contract
  publishes and the package reads as nothing.
- Whether a document that declares a rate limit through neither convention, staying silent, is worth a doctor finding at
  all, given the package has no way to know a limit exists in the first place.

...
| Remote `$ref` policy     | Domain allowlist, empty by default | **Decided** | A spec that can fetch a URL is a network client running with the app's credentials. Allowlisted hosts resolve; anything else is a clear exception. Rules: [`OPENAPI-SUPPORT.md`](./OPENAPI-SUPPORT.md#remote-references-and-the-domain-allowlist). Config key not yet chosen. |

...
| OpenAPI versions         | 3.0.x and 3.1.x                          | **Decided** | Both versions are *parsed*; a documented subset is *honored*. Modern design tools export 3.1. Feature coverage, and the version differences behind it, live in [`OPENAPI-SUPPORT.md`](./OPENAPI-SUPPORT.md).                                                                                                                                                                                                                                                                                                              |
...
| Remote `$ref` policy     | Domain allowlist, empty by default       | **Decided** | A spec that can fetch a URL is a network client running with the app's credentials. Allowlisted hosts resolve; anything else is a clear exception. A fetched reference is then treated as a **dependency**, never as a cache entry: the copy is vendored and committed, so the runtime never touches the network and an upstream change arrives as a reviewable diff. No lock file — git already is one. Rules: [`OPENAPI-SUPPORT.md`](./REMOTE-REFERENCES.md). Config keys, file names and command names not yet chosen. |
| YAML decoding            | `symfony/yaml`                           | **Decided** | Constraint `^7.0 \| ^8.0`, matching what Laravel 12/13 already installs. Direct rather than transitive because the pipeline decodes the document *before* the OpenAPI parser sees it — version detection, root shape and `$ref` cycle detection all work on the decoded array — so relying on the parser to keep depending on it would be relying on someone else's implementation detail.                                                                                                                                |
...
| Response DTOs            | `spatie/laravel-data`, or only its shape | Undecided   | Its `from($model)` ergonomics are the target. A dependency buys casting, validation and serialization; it also binds generated code to another package's API and release cycle. Reasoning: [`CODE-GENERATION.md`](./CODE-GENERATION.md#response-dtos).                                                                                                                                                                                                                                                                    |
